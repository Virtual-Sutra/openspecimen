...
- **Rollback** - downgrade detection auto-restores the matching backup (a downgrade is an older tagged release, `master` → any tag, or `master` → an older `master` snapshot); `rollback.yml` restores the prior deploy end-to-end (WAR + config + plugins + database) so the schema is always brought back with the app
